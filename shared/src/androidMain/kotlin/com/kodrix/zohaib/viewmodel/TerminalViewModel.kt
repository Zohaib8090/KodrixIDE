...
class TerminalViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun startLsp(file: java.io.File) {
        ...
        val client = LspClient(
            ...
            workingDir = projDir.absolutePath,
            env = env
        )
    ...
    private suspend fun doRequestCompletion(file: java.io.File, text: String, cursorOffset: Int) {
        ...
        try {
            ...
            if (response != null && response.containsKey("result") && response["result"] != null && response["result"] != kotlinx.serialization.json.JsonNull) {
                val result = response["result"]!!
                val items = mutableListOf<com.kodrix.zohaib.lsp.CompletionItem>()
                val jsonParser = kotlinx.serialization.json.Json { ignoreUnknownKeys = true }

                if (result is kotlinx.serialization.json.JsonObject && result.containsKey("items")) {
                    val list = jsonParser.decodeFromString(com.kodrix.zohaib.lsp.CompletionList.serializer(), result.toString())
                    items.addAll(list.items)
                } else if (result is kotlinx.serialization.json.JsonArray) {
                    result.forEach {
                        items.add(jsonParser.decodeFromString(com.kodrix.zohaib.lsp.CompletionItem.serializer(), it.toString()))
                    }
                }
    ...
    private fun parseProblem(line: String, root: java.io.File) {
        ...
        regex.find(line)?.let { match ->
            ...
            if (file.exists()) {
                val problem = IDEProblem(file.absolutePath, lineNum, colNum, msg, severity)
